...
# ray job submit --address=http://localhost:8265 --working-dir . -- python cnn_fmnist_train.py --data-dir=/fmnist --minio-endpoint=http://host.docker.internal:9000 --minio-access-key=ROOTUSER --minio-secret-key=CHANGEME123 --minio-bucket-name=fmnist --minio-object-name=train-images-idx3-ubyte.gz
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train F-MNIST using CNN")
...
if __name__ == "__main__":
    ...
    # Add default training configurations to the config dictionary
    training_config = {
        "batch_size": 64,          # Batch size
        "learning_rate": 0.001,    # Learning rate
        "num_epochs": 10,          # Number of epochs
    }
    config.update(training_config)  # Combine default configs with parsed args
